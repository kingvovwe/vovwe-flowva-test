    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      ...
      <Modal isOpen={modals.streak} onClose={() => setModals({ ...modals, streak: false })}>
        <div className="p-8 text-center flex flex-col items-center">
          ...
          <h3 className="text-2xl font-bold text-[#8b5cf6] mb-2">Level Up!</h3>
      ...
      <Modal isOpen={modals.stack} onClose={() => setModals({ ...modals, stack: false })}>
        <div className="p-8 text-center">
          <div className="w-16 h-16 bg-purple-100 rounded-full flex items-center justify-center mx-auto mb-4 text-[#8b5cf6]"><Share2 size={32} /></div>
          ...
          <button onClick={() => setModals({ ...modals, stack: false })} className="w-full py-3 bg-[#8b5cf6] text-white rounded-xl font-bold hover:bg-violet-600 transition-colors">Go to Tech Stack</button>
        </div>
      </Modal>
      ...
      <Modal isOpen={modals.reclaim} onClose={() => setModals({ ...modals, reclaim: false })}>
        <div className="p-6">
          ...
          <div className="space-y-4">
            <div><label className="text-xs font-bold text-gray-700 mb-1 block">Email used on Reclaim</label><input type="email" placeholder="user@example.com" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#8b5cf6] focus:ring-2 focus:ring-purple-50 transition-all" /></div>
          ...
          <div className="flex gap-3 mt-8"><button onClick={() => setModals({ ...modals, reclaim: false })} className="flex-1 py-3 text-gray-600 font-bold hover:bg-gray-100 rounded-xl transition-colors">Cancel</button><button className="flex-1 py-3 bg-[#8b5cf6] text-white font-bold rounded-xl hover:bg-violet-600 shadow-lg shadow-purple-200 transition-all">Submit Claim</button></div>
        </div>
      </Modal>

      <div>
        <h2 className="text-xl font-bold text-gray-900 mb-6 pl-4 border-l-4 border-[#8b5cf6]">Your Rewards Journey</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          
          {/* POINTS BALANCE */}
          <Card className="flex flex-col h-full">
            <CardHeader>
              <div className="flex items-center gap-2 text-[#8b5cf6] font-bold"><Trophy size={18} /><span>Points Balance</span></div>
            </CardHeader>
            <CardContent className="flex-1 flex flex-col justify-between p-6">
              <div className="flex items-center justify-between">
                <span className="text-5xl font-bold text-[#8b5cf6]">{profile?.points || 0}</span>
              ...
              <div className="mt-8">
                ...
                <div className="w-full bg-gray-100 rounded-full h-2"><div className="bg-[#8b5cf6] h-2 rounded-full transition-all duration-1000" style={{ width: `${Math.min(((profile?.points || 0) / 5000) * 100, 100)}%` }}></div></div>
          ...
          <Card className="flex flex-col h-full">
            ...
            <CardContent className="flex-1 flex flex-col justify-between p-6">
               <div className="mb-4 text-center">
                  <h3 className="text-4xl font-bold text-[#8b5cf6] mb-6 text-left">{streak} day</h3>
                      ...
                      return (
                        <div key={i} className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all border-2 ${
                          isFilled 
                            ...
                            : isCurrentDay
                              ? 'bg-white text-[#8b5cf6] border-[#8b5cf6]' // Current Day Outline
               ...
               <button 
                  ...
                  className={`w-full py-3 rounded-xl font-bold transition-all flex items-center justify-center gap-2 text-sm ${
                    ...
                    : 'bg-[#8b5cf6] hover:bg-[#7c3aed] text-white shadow-lg shadow-purple-100'
                  }`}
               >
          ...
          {/* FEATURED TOOL */}
            <Card className="flex flex-col h-full overflow-hidden border-0 shadow-xl">
                <div className="bg-gradient-to-r from-[#8b5cf6] to-[#ec4899] p-6 text-white relative">
                ...
                <div className="bg-white p-6 flex flex-col h-full justify-between">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-500 leading-relaxed">Reclaim.ai is an AI-powered calendar assistant that automatically schedules your tasks, meetings, and breaks to boost productivity. Free to try — earn Flowva Points when you sign up!</p>
                    </div>
                </div>
                <div className="flex gap-3 mt-auto">
                   <button className="bg-[#8b5cf6] text-white px-2 py-2.5 rounded-full text-sm font-bold hover:bg-[#7c3aed] transition-colors flex-1">Sign up</button>
      ...
      {/* EARN MORE POINTS */}
      <div>
        <h2 className="text-xl font-bold text-gray-900 mb-6 pl-4 border-l-4 border-[#8b5cf6]">Earn More Points</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           <Card className="h-full flex flex-col overflow-hidden group hover:ring-2 hover:ring-[#8b5cf6] hover:border-transparent transition-all duration-300">
              <div className="bg-white p-6 border-b border-gray-50">
                    <div className="flex items-center gap-3 text-[#8b5cf6] font-bold text-lg">
                ...
                <div className="bg-[#f8fafc] p-6 flex-1">
                    <p className="text-sm text-gray-600 leading-relaxed">Invite 3 friends by Nov 20 and earn a chance to be one of 5 winners of <span className="text-[#8b5cf6] font-bold">10,000 points</span>. Friends must complete onboarding to qualify.</p>
                </div>
           </Card>
           <Card className="h-full flex flex-col overflow-hidden">
                <div className="bg-white p-6 border-b border-gray-50 flex gap-4">
                    <div className="text-[#8b5cf6] mt-1">
                        <Share2 size={24} />
                    </div>
                    <div>
                        <h3 className="text-lg font-bold text-[#8b5cf6]">Share Your Stack</h3>
                ...
                <div className="bg-[#f8fafc] p-6 flex-1 flex items-center justify-between">
                    ...
                    <button onClick={() => setModals({ ...modals, stack: true })} className="bg-[#f3e8ff] text-[#8b5cf6] px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 hover:bg-[#ebd5fe] transition-colors"><Share2 size={16} /> Share</button>
                </div>
           </Card>
        </div>
      </div>
      ...
      {/* REFER & EARN  */}
      <div>
        <h2 className="text-xl font-bold text-gray-900 mb-6 pl-4 border-l-4 border-[#8b5cf6]">Refer & Earn</h2>
        <Card className="overflow-hidden bg-gray-50/50 border-none shadow-none">
           <div className="bg-white rounded-3xl border border-flowva-border overflow-hidden">
              <div className="p-8 pb-0">
                 <div className="flex items-center gap-3 text-[#8b5cf6] mb-1">
                ...
                <div className="p-8 pt-10">
                    <div className="flex justify-around mb-10">
                        <div className="text-center">
                            <p className="text-3xl font-bold text-[#8b5cf6] mb-1">0</p>
                    ...
                    <div className="mb-8">
                        ...
                        <div className="bg-white p-2 rounded-2xl border border-gray-200 flex items-center shadow-sm hover:border-[#8b5cf6] transition-colors">
                            <input readOnly value={`https://flowvahub.com/signup?ref=${profile?.referral_code || 'vovwe3070'}`} className="flex-1 bg-transparent px-4 text-sm text-gray-600 outline-none font-medium text-ellipsis" />
                            <button className="p-2 hover:bg-gray-50 rounded-xl text-[#8b5cf6] transition-all"><Copy size={20} /></button>
                        </div>
                    </div>
